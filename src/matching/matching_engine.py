...
class MatchingEngine:
    ...
    def __init__(self, driver, reject_keywords: List[str] = None, boost_keywords: List[str] = None):
    ...
    def _match_single(
        ...
        initial_keyword_score, initial_keyword_reason = self.keyword_filter.match(listing, product)
        ...
        if not listing.description:
            logger.info(f"[Match] Fetching description for potential match: {listing.title[:50]}...")
            listing.description = self.description_scraper.fetch_description(listing.url)
